<template>
    <div class="modi-container" v-if="article">
      <div class="article-container">
        ...
        <div class="card-container">
          <div class="card">
            <img :src="article.articleImgUrl" class="card-img-top" alt="이미지">
            <div class="card-body">
              <p class="card-text">
  ...
  try {
    ...
    article.value = response.data.article; // Accessing the 'article' field from the response
  } catch (error) {
    console.error('Error fetching article:', error);
      ...
      const deleteArticle = async () => {
        try {
          await axios.delete(`http://localhost:8080/manager/article/article/${articleId.value}`);
